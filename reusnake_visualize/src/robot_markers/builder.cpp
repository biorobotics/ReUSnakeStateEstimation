...
void Builder::Init() {
  ...
  for (size_t i = 0; i < links.size(); ++i) {
    ...
    ROS_INFO("init transform from %s to %s!", parent_name.c_str(), name.c_str());
...
void Builder::Build(MarkerArray* marker_array) {
  ...
  for (size_t i = 0; i < links.size(); ++i) {
    ...
    // std::cout << visual_transform.position.x << std::endl;
    // std::cout << "reach " << __FILE__ << __LINE__ << std::endl;
    // ROS_INFO("Find transform from %s to %s!", frame_id_.c_str(), name.c_str());
    bool success =
