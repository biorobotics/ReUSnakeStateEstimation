 ...
 * Notes: FK means Forward Kinematics
 *        This function is the same as the above FKinBody, but returns all intermediate tranforms 
 */
std::vector<Eigen::MatrixXd> FKinBodyInter(const Eigen::MatrixXd&, const Eigen::MatrixXd&, const Eigen::VectorXd&);
...
// a slight overloaded version of original IKinSpace to calculate only based on position, ignore orientation
bool IKinSpace(const Eigen::MatrixXd&, const Eigen::MatrixXd&, const Eigen::MatrixXd&, Eigen::VectorXd&, double);
